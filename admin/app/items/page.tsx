...
const ItemsManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      const eanTerm = filters.eanSearch.trim();

      if (eanTerm) {
        setDebouncedSearch(""); // Clear regular search when using EAN search
        setDebouncedEanSearch(eanTerm);
      } else {
        setDebouncedSearch(searchTerm);
        setDebouncedEanSearch("");
      }
    }, 300);
  ...
  const normalizeItem = (raw: any, fallbackRow: any) => {
    ...
    return {
      ...
      is_stock_item: r.is_stock_item ?? fb.is_stock_item ?? "N",
      stockEU: r.stockEU ?? fb.stockEU ?? 0,
      MSQ_EU: r.MSQ_EU ?? fb.MSQ_EU ?? 0,
      stockCN: r.stockCN ?? fb.stockCN ?? 0,
      MSQ_CN: r.MSQ_CN ?? fb.MSQ_CN ?? 0,
    };
  };
      ...
      try {
        ...
        switch (tab) {
          case "items": {
            ...
            if (debouncedEanSearch) {
              params.eanSearch = debouncedEanSearch;
            } else if (debouncedSearch) {
              params.search = debouncedSearch;
            }

            const res: any = await getItems(params, { refresh: force });
            data = res.data || [];
  ...
  useEffect(() => {
    ...
    if (itemIdParam && !showItemPreview && !isExplicitlyClosedRef.current) {
      const foundRow = tabData.items.find(
        (item) => String(item.id) === String(itemIdParam),
      );
      if (foundRow) {
        openItemPreview(foundRow);
      } else {
        openItemPreview({ id: parseInt(itemIdParam, 10) });
      }
    }
  }, [searchParams, tabData.items, showItemPreview]);
                ...
                <div className="text-xs text-gray-700 space-y-0.5 max-w-[250px] break-words">
                  {item.remark_ex || item.remark ? (
                    <div
                      ...
                      title={item.remark_ex || item.remark || ""}
                    >
                      {item.remark_ex || item.remark}
                    </div>
                  ) : null}
