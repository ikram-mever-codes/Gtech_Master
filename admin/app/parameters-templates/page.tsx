...
export default function ParametersTemplatesPage() {
  ...
  const handleDeleteTemplate = async () => {
    if (!confirm("Are you sure you want to remove the current Customer Document template?")) return;

    setDeletingTemplate(true);
    try {
      const res: any = await deleteDocumentTemplate("customer_doc_template");
      if (res && res.success) {
        setCustomerDocParam(res.data);
      }
    } catch (err) {
      console.error(err);
    } finally {
              ...
              <div className="flex items-center gap-3 border-b border-gray-100 pb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    Upload & manage main Customer Document template with date stamp
                  </p>
                </div>
              </div>

              {customerDocParam && customerDocParam.file_url ? (
                <div className="p-5 rounded-2xl border-2 border-emerald-100 bg-emerald-50/30 space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-gray-900 text-sm">
                          {customerDocParam.file_name || "Customer_Document_Template"}
                        </h4>
                        <p className="text-xs text-gray-500 font-mono mt-0.5">
                          {customerDocParam.file_type || "Template File"}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-white border border-emerald-100 text-xs font-medium text-gray-700">
                    <Clock className="w-4 h-4 text-emerald-600" />
                    <span>Upload Date Stamp:</span>
                    <span className="font-semibold font-mono text-gray-900">
                      {formatDateStamp(customerDocParam.uploaded_at)}
                    </span>
                  </div>

                  <div className="flex items-center gap-2 pt-1">
                    <a
                      href={resolveFileUrl(customerDocParam.file_url)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl transition-all flex items-center gap-1.5"
                    >
                      <Download className="w-3.5 h-3.5" /> Download Template
                    </a>

                    <label className="px-3.5 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold rounded-xl transition-all flex items-center gap-1.5 cursor-pointer">
                    ...
                    <button
                      ...
                      className="px-3.5 py-2 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-semibold rounded-xl transition-all flex items-center gap-1.5 ml-auto"
                    >
                ...
                <label className="border-2 border-dashed border-gray-200 hover:border-[#8CC21B] bg-gray-50/50 hover:bg-[#8CC21B]/5 rounded-2xl p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-all gap-3 group">
                  <div className="w-12 h-12 rounded-2xl bg-white border border-gray-200 group-hover:border-[#8CC21B] flex items-center justify-center text-gray-400 group-hover:text-[#8CC21B] shadow-sm transition-all">
                    {uploadingTemplate ? (
                  ...
                  <div>
                    <p className="text-sm font-bold text-gray-800">
                      Click to upload CustomerDocument Template
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                      Supports PDF, DOCX, XLSX, images (Max 20MB). Automatic date stamp will be generated.
                    </p>
                  </div>
                  <input
                    type="file"
