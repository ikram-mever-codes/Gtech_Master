...
export function DataTable<T>({
  ...
  const currentSortDirection = isControlled
    ? sortDirection
  ...
  return (
    <div className="overflow-x-auto min-h-[60vh] bg-white rounded-md shadow-lg border border-gray-200">
      <table className="w-full border-collapse">
        <thead
          className={headerClassName || "bg-gray-50 border-b border-gray-200"}
        >
          <tr>
            {columns.map((c, i) => {
          ...
          {loading ? (
            <tr>
              <td colSpan={columns.length} className="px-4 py-12 text-center">
                <div className="inline-flex items-center gap-3">
                  ...
                  <span className="text-gray-500 text-sm font-semibold font-poppins">
                    Loading data...
                  </span>
                </div>
              </td>
            ...
            <tr>
              <td
                ...
                className="px-4 py-12 text-center text-gray-500 text-sm font-medium font-poppins"
              >
                {emptyMessage}
              </td>
            </tr>
              ...
              if (isRowExpanded) {
                ...
                if (expandedRowIds instanceof Set) {
                  isExpanded = possibleKeys.some(
                    (k) =>
                      expandedRowIds.has(k) ||
                      expandedRowIds.has(String(k)) ||
                      (!isNaN(Number(k)) && expandedRowIds.has(Number(k))),
                  );
                } else if (Array.isArray(expandedRowIds)) {
                  isExpanded = possibleKeys.some(
                    (k) =>
                      expandedRowIds.includes(k) ||
                      expandedRowIds.includes(String(k)) ||
                      (!isNaN(Number(k)) && expandedRowIds.includes(Number(k))),
                  );
                }
              } else if (
                ...
                isExpanded = possibleKeys.some(
                  (k) =>
                    k === expandedRowId || String(k) === String(expandedRowId),
                );
              }
                ...
                >
                  ...
                  >
                    {columns.map((c, i) => (
                      <td
                        ...
                        className={
                          tdClassName ||
                          `px-3 py-3 text-${c.align || "left"} ${rowStyle?.color ? "" : "text-gray-700"}`
                        }
                        style={{
                          ...(c.width
                  ...
                  {isExpanded && renderRowDetails && (
                    <tr>
                      <td
                        colSpan={columns.length}
                        className="px-6 py-4 bg-gray-50/30 border-b border-gray-100"
                      >
