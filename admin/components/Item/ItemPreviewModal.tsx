...
import { parseFlexibleNumber, formatMax3Decimals } from "@/utils/decimal";
...
export const ItemPreviewModal: React.FC<ItemPreviewModalProps> = ({
  ...
  const handleSavePreview = async () => {
    ...
    try {
      ...
      toast.error(e.message || "Failed to update item", errorStyles);
    } finally {
                  ...
                  <Field label="TARIC">
                    {previewEdit ? (
                      <ReactSelect
                        ...
                        onChange={(opt: any) =>
                          patchPreview({
                            taric_id: opt ? opt.value : null,
                          })
                        }
                        isClearable
                        isSearchable
                        placeholder="Select TARIC..."
                        className="text-xs"
                        styles={{
                          control: (base) => ({
                            ...base,
                ...
                <Field label="RemarkEx">
                  {previewEdit ? (
                    <textarea
                      ...
                      onChange={(e) =>
                        patchPreview({ remark_ex: e.target.value })
                      }
                    />
                  ) : (
                  ...
                  {previewEdit ? (
                    <div className="flex items-center gap-1">
                      <DecimalInput
                        className={inputCls}
                        placeholder="0.00"
                        value={formatMax3Decimals(previewItem.price ?? 0) || ""}
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3 flex-shrink-0">
                              ...
                              {previewEdit && (
                                <button
                                  onClick={() =>
                                    deletePreviewAttachment(att.id)
                                  }
                                  className="text-rose-600 hover:text-rose-800"
                                  title="Delete"
                                >
              ...
              <div className="mt-6 pt-5 border-t border-gray-200">
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        if (newUrls.length > 0) {
                          ...
                          let gallery = (previewItem.pictures?.pixPath || "")
                            .split(",")
                            .filter(Boolean);
                          newUrls.forEach((url) => {
                ...
                {(() => {
                  ...
                  if (previewItem.pictures?.shopPicture)
                    picEntries.push({
                      url: previewItem.pictures.shopPicture,
                      label: "Shop Picture",
                    });
                  if (previewItem.pictures?.ebayPictures)
                    picEntries.push({
                      url: previewItem.pictures.ebayPictures,
                      label: "eBay Picture",
                    });
                  (previewItem.pictures?.pixPath || "")
                    .split(",")
                    .filter(Boolean)
                    .forEach((u: string, i: number) =>
                      picEntries.push({ url: u, label: `Gallery ${i + 1}` }),
                    );
                  if (picEntries.length === 0) return null;
                  return (
