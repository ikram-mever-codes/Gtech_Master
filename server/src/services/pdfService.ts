...
/**
 * Generates a Purchase Order PDF in memory and returns it as a Buffer
 */
export async function generatePurchaseOrderPDFBuffer(so: SupplierOrder): Promise<Buffer> {
    ...
    try {
        doc.fontSize(16).fillColor("#333333").font("Helvetica-Bold").text("GTech Industries Limited", 40, 30, { align: "right" });
        doc.moveTo(40, 55).lineTo(555, 55).strokeColor("#333333").lineWidth(0.8).stroke();
        doc.fillColor("#666666").font("Helvetica").fontSize(8).text("Engineering ✔ Design ✔ Manufacturing ✔", 40, 60, { align: "right" });
        ...
        doc.fillColor("#000000").font("Helvetica-Bold").fontSize(20).text("Purchase Order", 0, 130, { align: "center" });
        ...
        const drawGridRow = (y: number, label1: string, val1: string, label2?: string, val2?: string, height: number = rowH) => {
            ...
            doc.fillColor("#000000").font("Helvetica-Bold").fontSize(10).text(label1, col1 + 8, y + (height / 2 - 4));
            const val1Width = label2 ? col3 - col2 : 555 - col2;
            doc.rect(col2, y, val1Width, height).fillAndStroke("white", "#000000");
            doc.fillColor("#000000").font("Helvetica").fontSize(10).text(val1, col2 + 8, y + (height / 2 - 4), { width: val1Width - 12, height: height - 4 });

            if (label2) {
                doc.rect(col3, y, col4 - col3, height).fillAndStroke("#f2f2f2", "#000000");
                doc.fillColor("#000000").font("Helvetica-Bold").fontSize(10).text(label2, col3 + 8, y + (height / 2 - 4));
                doc.rect(col4, y, 555 - col4, height).fillAndStroke("white", "#000000");
                doc.fillColor("#000000").font("Helvetica").fontSize(10).text(val2 || "", col4 + 8, y + (height / 2 - 4), { width: 555 - col4 - 12, height: height - 4 });
            }
        };
        ...
        for (const item of items) {
            ...
            if (attachments.length > 0 || qualityCriteria.length > 0) {
                checkSpace(30);
                doc.fillColor("#000000").font("Helvetica-Bold").fontSize(11).text(`Additional Details for: ${item.item?.item_name}`, cols[0], itemY);
                itemY += 20;
        ...
        checkSpace(100);
        doc.fontSize(10).font("Helvetica-Bold").text("Payment Terms", cols[0], itemY);
        itemY += 18;
        ...
        checkSpace(80);
        doc.fontSize(10).font("Helvetica-Bold").text("Bank Information", cols[0], itemY);
        itemY += 18;
        doc.fontSize(9).font("Helvetica");
        ...
        doc.fontSize(10).font("Helvetica-Bold").text("Signatures of Representatives:", cols[0], sigY - 35);
        doc.fontSize(9).font("Helvetica").text(so.supplier?.company_name || "", cols[0], sigY - 18);
        doc.moveTo(cols[0], sigY + 40).lineTo(cols[0] + 180, sigY + 40).stroke();
        doc.fontSize(9).font("Helvetica").text("GTech Industries Limited", 350, sigY - 18);
        ...
        doc.fontSize(9).font("Helvetica").text("Joschua Grenzheuser", 350, sigY + 48, { align: "center", width: 165 });
