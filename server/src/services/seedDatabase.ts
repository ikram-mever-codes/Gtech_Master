...
export const seedDatabase = async () => {
    try {
        ...
        const gtechCompanyRepository = AppDataSource.getRepository(GtechCompany);
        const existingGtechCompanies = await gtechCompanyRepository.count();
        if (existingGtechCompanies === 0) {
            console.log("🏢 Seeding default GTech companies...");
            const defaultCompanies = [
                {
                    legal_name: "GTech Industries Limited, HK",
                    display_name: "GTech Hong Kong",
                    country: "Hong Kong",
                },
                {
                    legal_name: "GTech Industries Sp. z o.o.",
                    display_name: "GTech Poland",
                    country: "Poland",
                },
                {
                    legal_name: "GTech Industries GmbH",
                    display_name: "GTech Germany",
                    country: "Germany",
                },
                {
                    legal_name: "GTech Industries Service Limited",
                    display_name: "GTech Cyprus",
                    country: "Cyprus",
                },
            ];
            for (const comp of defaultCompanies) {
                await gtechCompanyRepository.save(gtechCompanyRepository.create(comp));
            }
