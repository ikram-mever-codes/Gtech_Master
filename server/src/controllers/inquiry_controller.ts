...
export class InquiryController {
  ...
  private async resolveOrCreateDraftItemForRequest(
    ...
    const fields: Partial<Item> = {
      ...
      // cat_id intentionally NOT set here on the update-existing branch —
      // an already-created Item's category is never overwritten by this
      // path. It's forced only at creation time, below.
    };
    ...
    // Category is always PRO on creation — unconditional, not a fallback.
    const proCatId = await resolveProCategoryId(queryRunner);
    // EAN is always freshly autogenerated and unique on creation.
    const ean = await generateUniqueEAN(queryRunner);
    ...
    const created = itemRepo.create({
      ...
      cat_id: proCatId,
      ean,
      // Company — the Inquiry's own customer, passed in by the caller.
  ...
  async updateInquiry(request: Request, response: Response) {
    ...
    try {
      ...
      const inquiryRepo = queryRunner.manager.getRepository(Inquiry);
      const requestRepo = queryRunner.manager.getRepository(RequestedItem);
      const contactPersonRepo =
        queryRunner.manager.getRepository(ContactPerson);
      const starBusinessDetailsRepo =
        queryRunner.manager.getRepository(StarBusinessDetails);

      const existingInquiry = await inquiryRepo.findOne({
        where: { id },
        relations: [
          "customer",
          "contactPerson",
      ...
      if (!inquiryNo) {
        ...
        await inquiryRepo.update(id, { inquiryNo });
      }

      let contactPerson = null;
      if (contactPersonId) {
        contactPerson = await contactPersonRepo.findOne({
          where: { id: contactPersonId },
        });
        if (!contactPerson) {
          await queryRunner.rollbackTransaction();
      ...
      await inquiryRepo.update(id, updateData);
      ...
      if (requests && Array.isArray(requests)) {
        ...
        if (existingInquiry.requests && existingInquiry.requests.length > 0) {
          await requestRepo.remove(existingInquiry.requests);
        }
          ...
          if (starBusinessDetails) {
            const requestEntities = await Promise.all(
              requests.map(async (reqData: any, index: number) => {
                ...
                // doesn't carry the itemId for this line. Category (PRO),
                // EAN (autogenerated + unique), and company (this
                // Inquiry's customer) are all applied inside that method
                // on the creation branch only — never on an update to an
        ...
        await inquiryRepo.update(id, { total_potential_k_eur });
      }
