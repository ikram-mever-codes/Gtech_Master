...
const syncInvoiceRecord = async (
  ...
  for (const oi of items) {
    ...
    const unitPrice =
      oi.eur_special_price || oi.price || oi.rmb_special_price || 0;
    const netPrice = quantity * unitPrice;

    let itemTaxRate =
      oi.item?.taric?.duty_rate !== undefined
        ? Number(oi.item?.taric?.duty_rate)
        : 19;
  ...
  invoice.outstandingAmount = Math.max(
    0,
    grossTotal - (invoice.paidAmount || 0),
  );
...
export const getAllCargos = async (
  ...
  try {
    ...
    const dataWithCounts = cargos.map((c: any) => {
      const cargoNoDisplay = c.cargo_no && c.cargo_no.trim() ? c.cargo_no : `Cdraft-${c.id}`;
      return {
        ...c,
  ...
  try {
    ...
    if (!rawCargoNo || rawCargoNo === defaultPrefix || isCustomText) {
      ...
      try {
        ...
        console.warn(
          "Could not generate draft cargo number:",
          err,
        );
      }
    }
...
export const assignOrdersToCargo = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    if (validOrderIds.length > 0 && !isSplitMove) {
      ...
      for (const o of ordersForOldCargos) {
        ...
        const items = await orderItemRepo.find({ where: { order_id: o.id } });
    ...
    const existing = await cargoOrderRepo.find({
      where: { cargo_id: cargo.id },
    });
...
export const removeOrderFromCargo = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const getCargoOrders = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
