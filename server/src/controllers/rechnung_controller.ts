  ...
  try {
    ...
    const rechnung = rechnungRepo.create({
      ...
      date_delivery:
        ...
        ((auftrag as any).delivery_date
          ? String((auftrag as any).delivery_date)
          : undefined),
...
export const getRechnungById = async (
  ...
  try {
    ...
    const title = rechnung.title || linkedAuftrag?.title || undefined;
    const resolvedDeliveryDate =
...
export const downloadRechnungPdf = async (
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="${downloadFileName}"; filename*=UTF-8''${encodeURIComponent(downloadFileName)}`,
    );
