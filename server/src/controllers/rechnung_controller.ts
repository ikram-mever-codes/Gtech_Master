...
export async function attachPaymentsAndRksToRechnungen(rechnungen: Rechnung[]): Promise<void> {
...
export const downloadRechnungPdf = async (
  ...
  try {
    ...
    await generateGtechDocumentPdf({
      ...
      outstandingAmount: (pdfPayments.length > 0 || pdfRks.length > 0) ? outstandingAmount : undefined,
      taxProfile:
