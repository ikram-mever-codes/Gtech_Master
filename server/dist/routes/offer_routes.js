...
router.post("/line-items/:lineItemId/price-matrix", offerController.addPriceMatrixEntry.bind(offerController));
...
router.post("/:offerId/pricing-mode", offerController.togglePricingMode.bind(offerController));
// Re-apply the offer's tier template across all its line items
router.post("/:offerId/sync-price-matrix", offerController.syncPriceMatrixAcrossOffer.bind(offerController));
// Delete one tier (quantity column) from every line item on the offer
router.delete("/:offerId/price-column", offerController.deletePriceColumn.bind(offerController));
// Paste-in matrix import — the client's primary entry point
router.post("/:offerId/paste-matrix", offerController.pasteMatrixPrices.bind(offerController));
// ---------------------------------------------------------------------------
