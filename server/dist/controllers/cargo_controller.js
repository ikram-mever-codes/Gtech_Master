...
const generateInvoicesForOrders = (orderIds_1, ...args_1) => __awaiter(void 0, [orderIds_1, ...args_1], void 0, function* (orderIds, cargoIds = []) {
    ...
    try {
        ...
        for (const cargoNo of Array.from(cargoNumbers)) {
            const cargo = yield cargoRepo.findOne({
        ...
        for (const orderNo of Array.from(orderNumbers)) {
            const existingInvoice = yield invoiceRepo.findOne({
...
const syncInvoiceRecord = (orderNumber, items, customer, invoiceRepo, invoiceItemRepo, customerRepo) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let invoice = yield invoiceRepo.findOne({
        where: { orderNumber },
        relations: ["items"],
    });
    ...
    for (const oi of items) {
        ...
        let itemTaxRate = ((_b = (_a = oi.item) === null || _a === void 0 ? void 0 : _a.taric) === null || _b === void 0 ? void 0 : _b.duty_rate) !== undefined
            ? Number((_d = (_c = oi.item) === null || _c === void 0 ? void 0 : _c.taric) === null || _d === void 0 ? void 0 : _d.duty_rate)
            : 19;
...
const getAllCargos = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (cargoIds.length > 0) {
            const orderItemRepo = database_1.AppDataSource.getRepository(order_items_1.OrderItem);
            const counts = yield orderItemRepo
        ...
        const dataWithCounts = cargos.map((c) => (Object.assign(Object.assign({}, c), { assignedItemsCount: itemCountsMap[c.id] || 0 })));
        res.status(200).json({
            success: true,
            data: dataWithCounts,
            pagination: {
...
const createCargo = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!cargoData.cargo_no || !cargoData.cargo_no.trim()) {
            try {
                cargoData.cargo_no = yield number_sequence_service_1.NumberSequenceService.getNextNumber("cargo");
            }
            catch (err) {
...
const assignOrdersToCargo = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (validOrderIds.length === 0) {
            res
                .status(400)
                .json({ success: false, message: "No valid orderIds provided" });
            return;
        }
        const cargoRepo = database_1.AppDataSource.getRepository(cargos_1.Cargo);
        const cargo = yield cargoRepo.findOne({ where: { id: Number(id) } });
