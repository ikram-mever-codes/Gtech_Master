...
class OfferController {
    ...
    generateOfferNumber() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { NumberSequenceService } = yield Promise.resolve().then(() => __importStar(require("../services/number_sequence_service")));
                return yield NumberSequenceService.getNextNumber("offer");
            }
            catch (e) {
                const date = new Date();
                ...
                try {
                    warehouseItems = yield warehouseRepository.find({
                        where: itemIdDEs.length
                            ? [
                                { ItemID_DE: (0, typeorm_1.In)(itemIdDEs) },
                                { item_id: (0, typeorm_1.In)(orderedItems.map((it) => it.id)) },
                            ]
                            : { item_id: (0, typeorm_1.In)(orderedItems.map((it) => it.id)) },
                    });
                }
                catch (e) {
                    console.warn("warehouse_items table not available while creating offer from item:", e === null || e === void 0 ? void 0 : e.message);
                }
                const getDeNo = (it) => {
                    var _a;
                    const warehouseMatch = warehouseItems.find((wi) => it.ItemID_DE && wi.ItemID_DE === it.ItemID_DE) || warehouseItems.find((wi) => wi.item_id === it.id);
                    return (warehouseMatch === null || warehouseMatch === void 0 ? void 0 : warehouseMatch.item_no_de) || ((_a = it.parent) === null || _a === void 0 ? void 0 : _a.de_no) || "";
                };
                let customer = null;
                if (body.customerId) {
                        ...
                        try {
                            warehouseItems = yield warehouseRepository.find({
                                where: itemIdDEs.length
                                    ? [
                                        { ItemID_DE: (0, typeorm_1.In)(itemIdDEs) },
                                        { item_id: (0, typeorm_1.In)(sourceItems.map((it) => it.id)) },
                                    ]
                                    : { item_id: (0, typeorm_1.In)(sourceItems.map((it) => it.id)) },
                            });
                        }
                        catch (e) {
                            console.warn("warehouse_items table not available while backfilling offer line items:", e === null || e === void 0 ? void 0 : e.message);
                        }
                        const getDeNo = (it) => {
                            var _a;
                            const warehouseMatch = warehouseItems.find((wi) => it.ItemID_DE && wi.ItemID_DE === it.ItemID_DE) || warehouseItems.find((wi) => wi.item_id === it.id);
